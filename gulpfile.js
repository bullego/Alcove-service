...
gulp.task('animate', function() {
	return gulp.src(devPath.src + '/../node_modules/animate.css/animate.min.css')		
		.pipe(gulp.dest(devPath.src + devPath.css.dest));
});
...
gulp.task('js-libs', ['js-libs-node-modules'], function() {
	return gulp.src([
		...
		//devPath.src + devPath.jsLibs.src + '/popper.js/dist/popper.min.js', // Подключаем, если этого требуют компоненты bootstrap		
		devPath.src + devPath.jsLibs.src + '/slick-carousel/slick/slick.min.js',
		devPath.src + devPath.jsLibs.src + '/jquery.maskedinput.min.js'	
		])
...
gulp.task('js-libs-node-modules', function() {
	return gulp.src([devPath.src + devPath.jsLibs.src + '/custom-bootstrap.js',
		devPath.src + devPath.jsLibs.src + '/wow.js'])
		.pipe(rigger()) //модуль для склейки нескольких файлов в один через конструкцию "//= путь"
